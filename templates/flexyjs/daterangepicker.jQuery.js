...
//			allDatesAfter: 'All Dates After', 
		...
		onOpen: function(){},
		onChange: function(rangeA,rangeB){},
	...
	var rpPresets = (function(){
		ul = jQuery('<ul class="ui-widget-content" style="display:none"></ul>').appendTo(rp);
	...
	function hideRP(){
		if(rp.data('state') == 'open'){ 
			...
			rp.fadeOut(300);
			options.onClose($(rangeInput[0]).attr('id'),rangeA,rangeB); 
		}
	}
	function toggleRP(){
	...
	//picker divs
	var rpPickers = jQuery('<div style="clear:both"></div><div class="ranges ui-widget-header ui-corner-all ui-helper-clearfix"><div class="range-start"><span class="title-start">Start Date</span></div><div class="range-end"><span class="title-end">End Date</span></div></div>').appendTo(rp);
	...
	rpPickers.find('.range-end').datepicker("option","onSelect",function(selectedDate, inst){
			rangeA = fDate( rpPickers.find('.range-start').datepicker('getDate') );
			rangeB = fDate( rpPickers.find('.range-end').datepicker('getDate') );
			//alert(rangeA+'===ha ha ha====='+rangeB);															 
			if(!(new Date(selectedDate)>=rpPickers.find('.range-start').datepicker("getDate"))){
				jqueryAlert("End range should be higher than start range");
				doneBtn.hide();
				return false;
			}else{
				doneBtn.show();
