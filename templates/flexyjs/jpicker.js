...
(function($, version)
{
              ...
              container.css( // positions must be set and display set to absolute before source code injection or IE will size the container to fit the window
                {
                  left: window.position.x == 'left' ? '-526px' : window.position.x == 'center' ? '-259px' : window.position.x == 'right' ? '0px' : window.position.x == 'screenCenter' ?
                    (($(document).width() >> 1) - 204) - $this.next().offset().left + 'px' : window.position.x,
                  position: 'absolute',
                  top: window.position.y == 'top' ? '-490px' : window.position.y == 'center' ? '-158px' : window.position.y == 'bottom' ? '25px' : window.position.y
                });
  ...
  $.fn.jPicker.defaults = /* jPicker defaults - you can change anything in this section (such as the clientPath to your images) without fear of breaking the program */
      ...
      images:
        {
          clientPath: 'http://demo1.afixiindia.com/recruitsure/templates/default/images_jpiker/', /* Path to image files */
          colorMap:
          {
            width: 256,
            height: 256,
