...
#####################################################################################################
	function _employeeList(){
		...
		$this->_output['field'] = array("name"=>array("Name",1),"job_title_name"=>array("Title",1),"joined_date"=>array("Hire date",1),"work_email"=>array("Work e-mail",1),"work_phone"=>array("Work Phone",1),"emp_status"=>array("Status",0));
	...
	function _insertEmployee(){
		if($_SESSION['id_user']==$_SESSION['id_company']){
			...
			if($employee_id){
				...
				$salary['current_salary']=$arr['salary'];
				$salary['currency']=$arr['currency'];
				$salary['pay_frequency']=$arr['pay_frequency'];
